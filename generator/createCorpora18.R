...
listFilesSortSize <- function(csvfolder,
                              filere,
                              fn = T,
                              rec = T) {
...
processParticipants <- function(one) {
  ...
  if ("age" %in% names(partdf) && sum(!is.na(partdf$age)) > 0) {
    ...
    ye = !is.na(partdf$Y)
    partdf$agemonth[ye &
                      !mon] =  partdf$agemonth[ye &
                                                 !mon] + 6 # if missing, put age in middle of year
    day = !is.na(partdf$D)
    ...
    if (!file.exists(newfile) | force) {
      ...
      if (file.exists(fulfile) && str_detect(fulfile, "xml")) {
        ...
        if ("id" %in% names(partdf)) {
          ...
          if ("who" %in% names(alldf) &&
              length(unique(alldf$who)) < length(unique(partdf$id))) {
            print("bind prev")
            #         print(head(alldf))
            #          print("new")
...
createUtteranceCorpora <- function(csvfolder) {
  flist = listFilesSortSize(csvfolder,
                            ".+?_.+?_Word.rds",
                            fn = T,
                            rec = F)
  print(paste("\n\n@@ change Word To Utterance", length(flist)))
  funclist = c('writeUtteranceCorpora',
               'word2sent',
               'pasteCol',
               'readFileLoop',
               'safeSave')
...
if (mode == 10 || mode == 0) {
  system.time(createNgramLang("ngramdir"))
  gc()
}
print("finished createNgramLang")

if (mode == 11 || mode == 0) {
  system.time(createNgramLang("ngramdir", TRUE))
  gc()
}
