...
- You can download a file at the corpus, language, and language group level. You will be downloading the whole file (regardless of max size), so it can take a while to download. You should be able to open csv files in excel, but if you need unicode encoded characters, then first load it in R and save using write.xlsx.  There is R code for doing this at the bottom of this page.
...
```{r, echo=FALSE}
pre(htmltools::includeText('storage/savexlsx.R'))
```
