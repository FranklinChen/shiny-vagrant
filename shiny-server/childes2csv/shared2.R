...
searchForCorpusFile <- function(){
  ...
  if (input$lang == ignore){
    csvfile <-  paste(gstorage,input$langGroup,"_",input$rowunit,".rds",sep="")
  }else{
    if (input$corpus == ignore){
      csvfile <-  paste(gstorage,input$langGroup,"_",input$lang,"_",input$rowunit,".rds",sep="")
    }else{
      csvfile <-  paste(gstorage,input$langGroup,"_",input$lang,"_",input$corpus,"_",input$rowunit,".rds",sep="") 
    }
  }
...
output$downloadDataLangGroup <- downloadHandler(
  ...
  content <- function(file) {
    geturl = paste(gstorage,input$langGroup,"_",input$rowunit,".rds",sep="")
...
output$downloadDataLang <- downloadHandler(
  ...
  content <- function(file) {
    geturl = paste(gstorage,input$langGroup,"_",input$lang,"_",input$rowunit,".rds",sep="")
    wholecsv <- readRDS(geturl)
    #    print(geturl)
#    file.copy(geturl,file)
#    write.csv.utf8.BOM(wholecsv, file)
...
output$downloadDataCorp <- downloadHandler(
  ...
  content <- function(file) {
    geturl = paste(gstorage,input$langGroup,"_",input$lang,"_",input$corpus,"_",input$rowunit,".rds",sep="")
    wholecsv <- readRDS(geturl)
  #  print("read wholecsv")
    #    print(geturl)
    #    file.copy(geturl,file)
