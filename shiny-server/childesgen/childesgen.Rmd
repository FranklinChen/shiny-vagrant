...
- Select a corpus to use (program will be slower when you first use a new corpus).
- Use the checkboxes to select the next word in the sentence.
- Transitional probabilities for the last word based on the previous context are shown. The choices are ordered by probabilities, so the top choice is the most likely continuation.
...
- If you select the ----- in the language or corpus field, you can use ngrams from all of the corpora under that language group or language. 
...
ga('send', 'pageview');")),
    tags$script(src='https://www.google-analytics.com/analytics.js'),
        fluidRow(column(3,selectInput("langGroup", "Language Group: ", 
                             choices = c("Japanese"), width='90%'))
                 ,column(3,selectInput("lang", "Language: ",
                              choices = c("Miyata"),width='90%'))
                 ,column(3,selectInput("corpus", "Corpus: ", 
                              choices = c("Tai"),width='90%'))
  ...
  server = function(input, output, session) {
    ...
    dd <- readFileDir("English","Thomas","-----",addignore=TRUE)
...
#      print(head(df2))
        ...
        uniqfreq = length(unique(df2$freq)) 
        if (uniqfreq > 16){
          numopt = 2
          if (uniqfreq > 24){
            numopt = 3
          }
          uniqval = unique(df2$freq)
          med = uniqval[length(uniqval)/2]
      #    print(med)
          mid = which(df2$freq==med)
     #     print(mid)
          df3 = df2[c(1:(numopt*2+1), (mid-numopt):(mid+numopt), (lendf2-(numopt*2)):lendf2),]
        }else{
          df3 = df2
        }
      #  print("df3")
   ...
   #   print(head(df3))
    ...
    getFileName <- function(nopath,gram){
...
- The transitional probabilities are computed by taking the frequency of the last three words and diving by the frequency of the bigram composed of the first two words.  For the utterance "I want a drink", the frequency of "want a drink" will be divided by the frequency of "want a".  This tells us out of all of the case of "want a", what proportion were followed by "drink".  If the sequence is only two words, then bigram frequency is divided by the unigram frequency of the first word.

- The first few clicks will be slow to update, because files need to be loaded, but it will be faster later.
