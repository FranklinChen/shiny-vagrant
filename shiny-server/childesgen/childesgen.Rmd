...
- Type a word sequence in the textbox
- Use the checkboxes to select the next word.  Transitional probabilities are provided based on the context
...
ga('send', 'pageview');")),
    tags$script(src='https://www.google-analytics.com/analytics.js'),
        fluidRow(column(3,selectInput("langGroup", "Language Group: ", 
                 ...
                 ,column(2,h1(" "),actionButton("reset", "Reset")))
         ,fluidRow(column(7,textInput('wordseq',"Word Sequence","I want",width='1400px')))
         ,fluidRow(column(7,textOutput("bigram"),textOutput("trigram")))
         ,fluidRow(column(4,checkboxGroupInput("selectnextword", label = "Select the next word")))
     ),
  server = function(input, output, session) {
    source('../childes2csv/shared3.R', local=TRUE)
    values$updateFilter = TRUE
    values$wordseq = c("I","want")
    dd <- readFileDir("Eng-UK","Thomas","----")
    values$n1 = "NULL"
    values$n2 = "NULL"
    values$n3 = "NULL"
    values$n4 = "NULL"
 ...
 #     print(tdf)
      ...
      values$bigram = paste("Product of bigram probabilities",paste(signif(bigram),collapse=" * "),"=", signif(prod(bigram, na.rm=F)))
}
...
#      print(n2$freq[n2$ngrams %in% tdf$bi])
      ...
      values$trigram = paste("Product of trigram probabilities",paste(signif(trigram),collapse=" * "),"=", signif(prod(trigram, na.rm=F)))
      }
    })
