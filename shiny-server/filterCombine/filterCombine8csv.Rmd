...
#preread <- read.csv("../storage/actualcsv/Eng-UK_Belfast_Barbara_Word.csv")
```
...
shinyApp(
 ...
 #   ,tags$td("")
      ,fluidRow(
        ...
        ,column(2, selectInput("preset", "Preset: ", choices=c(ignore,"U-shape Curve"))
                ,h5("Filtered CSV")
 ...
 #         updateSelectInput(session, "langGroup",selected="Eng-UK")
  #        updateSelectInput(session, "lang",selected="Thomas")
   #       updateSelectInput(session, "corpus",selected=ignore)
     #    isolate({
          values$fulltable=readRDS("gocome3.rds")
   ...
   observeEvent(input$source1,  ignoreInit=T,{
     ...
     #   print(head(colval))
      ...
      recode()
       ...
       observe({
         ...
         input$operation
         isolate({
         newcol = paste("op",input$varA,input$varB,sep="_")
     ...
     #     VarA = values$table[input$varA]
          foundA = TRUE
        }
       if (input$varB %in% names(values$table)){
         assign("VarB", values$table[input$varB], envir = e1)
     #    VarB = values$table[input$varB]
         foundB = TRUE
       }
 #       if (length(values$table[input$varB]) == length(values$table[input$varA]) && length(values$table[input$varA]) > 1){
        if (foundA && foundB){
   #     if (eval(length(VarA)==length(VarB), envir = e1)){
          print("env")
 #       if (length(VarB) == length(VarA) && length(VarA) > 1){
          ...
          numcol = which(newcol == names(values$table))
          values$table = values$table[,c(numcol,setdiff(1:length(values$table), numcol))]
       #   saveRDS(values$table,"ThomasPastTense.rds")
         cho = c(ignore,names( values$table))
          updateSelectInput(session, "dv", choices = cho,selected=newcol)
          updateSelectInput(session, "grp1", choices = cho)
          updateSelectInput(session, "grp2", choices = cho)
...
- This work is part of the toolkit project within the ESRC International Centre for Language and Communicative Development (LuCiD) (http://www.lucid.ac.uk/, ESRC grant [ES/L008955/1]).  Please contact <a href="https://sites.google.com/site/sentenceproductionmodel/cv" class="uri">Franklin Chang</a> with any questions.
