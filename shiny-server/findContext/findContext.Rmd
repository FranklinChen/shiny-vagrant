...
#    })
 ...
 #     print("observe")
      isolate({
        ...
        values$debug <-input$col1
      
        if (input$col1 != ignore && input$col1 %in% names(values$table) ){
          values$table[,input$col1]=as.character(values$table[,input$col1])
          check = !is.na(values$table[,input$col1]) & str_detect(values$table[,input$col1],input$search1)
          values$debug <- sum(check)
 #         values$table <- values$table[!is.na(values$table[,input$col1]) && check,]
          values$table <- values$table[check,c("role","w","file")]
